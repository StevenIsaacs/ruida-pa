...
Since rpascript represents the current understanding  of the Ruida controller protocol and is the result of the the combined efforts of several people who have contributed their discoveries over a period of several years. Little by little this understanding has grown but is by no means complete at this time. Because of this rpascript is subject to change as more discoveries are made. This can lead to revision hashing for applications needing to communicate with Ruida controllers of different types.  The goal of GlueScript is to insulate applications from as much of this thrashing and protocol idiosyncrasies as possible..
...
│   ├─────────────────┤  ├───────────────────────────────────┤   │
│   │ declare_job()   │  │ REF_POINT_MACHINE                │   │
│   │ declare_layer() │  │ LAYER_COLOR Layer:0 ...           │   │
│   │ move_xy_to()    │  │ MOVE_NEAR_XY nearX=... nearY=...  │   │
...
4. **Add operations** — Within each layer, add moves, cuts, and power
   ...
   (`home`, `home_z`, `home_u`) are live-only too: `home()` jogs the XY
   axes to the current origin reference, while `home_z`/`home_u` home the
   machine immediately against the connected controller. See Section 5.)
...
```python
# Near form — target within -8.192..+8.191 mm of current position
driver.move_xy_to(3.5, 2.0)
# Produces: MOVE_NEAR_XY nearX=3.500mm nearY=2.000mm

# Far form — target beyond the near range
driver.move_xy_to(100.0, 50.0)
# Produces: MOVE_FAR_XY X=100.000mm Y=50.000mm
```
...
```python
# Near form — target within -8.192..+8.191 mm of current position
driver.cut_xy_to(3.5, 2.0)
# Produces: CUT_NEAR_XY nearX=3.500mm nearY=2.000mm

# Far form — target beyond the near range
driver.cut_xy_to(200.0, 100.0)
# Produces: CUT_FAR_XY X=200.000mm Y=100.000mm
```
...
`set_mode()` can switch the current layer to `IMAGE`/`DEPTHMAP` mid-stream, so
`power()` becomes usable within a layer that was declared in another mode.
...
Switch the current layer to another layer mode mid-stream. Changes the mode
used by downstream gating (e.g. `power()` only fires in `IMAGE`/`DEPTHMAP`
layers) and forces the mode's overscan override when the effective overscan
differs.
...
| Parameter | Type | Description |
|-----------|------|-------------|
| `mode` | `str` | Layer mode: `VECTOR`, `RASTER`, `DITHER`, `IMAGE`, `DEPTHMAP` |

The rpascript is **change-gated**: a switch to the layer's current mode emits
only the transcript line and no rpascript at all. A mid-stream mode switch has
**no direct rpascript line** beyond overscan-forcing: switching to `VECTOR`
re-emits `OVERSCAN_OFF` (when the effective overscan differs), and switching
to a non-override mode (`RASTER`/`DITHER`/`IMAGE`/`DEPTHMAP`) emits nothing —
an explicitly set overscan persists across the switch and must be cancelled
with `set_overscan("NONE")`. A declared overscan is **not** restored when
returning to a non-override mode; the caller restores it via `set_overscan`.

```python
driver.set_mode('RASTER')   # no rpascript (RASTER has no overscan override)
driver.set_mode('VECTOR')   # Produces: OVERSCAN_OFF (VECTOR forces NONE)
```

**Constraints** (a layer must be declared first — that raises `ValueError` on
the direct path, surfacing as `RuntimeError` when re-staging a persisted
transcript):

- A layer must be declared first.
- `mode` must be one of `VECTOR`, `RASTER`, `DITHER`, `IMAGE`, `DEPTHMAP`.

`set_mode()` is a **saved-job command** — it is persisted to, and replayed
from, `.cglu` files.

#### `set_overscan(overscan: str)`

Set the overscan mode for the current layer at the call position. Unlike
`declare_layer()`, this method does **not** consult the `VECTOR` mode override:
the caller has opted into explicit per-section overscan, so the requested mode
is emitted verbatim into the current layer's action block at the call position.
This allows per-section overscan within a single layer.

**Parameters:**

| Parameter | Type | Description |
|-----------|------|-------------|
| `overscan` | `str` | Overscan mode: `NONE`, `X`, `X_BI`, `Y`, `Y_BI`, `XY` |

The rpascript is **change-gated**: setting the effective overscan to its
current value emits only the transcript line.

```python
driver.set_overscan('X')   # Produces: OVERSCAN_H_UNI
driver.set_overscan('NONE')  # Produces: OVERSCAN_OFF
```
...
- A layer must be declared first.
- `overscan` must be one of `NONE`, `X`, `X_BI`, `Y`, `Y_BI`, `XY`.

`set_overscan()` is a **saved-job command** — it is persisted to, and replayed
from, `.cglu` files.
...
Set the move speed for the currently active layer, in mm/s. Expands to a `#`
comment placeholder in the layer's action block:

```python
driver.move_speed(300.0)
...
Set the laser pulse width in microseconds for the currently active layer. The
maximum pulse width is 1mS (1000µs); a duration above 1000 logs a warning
(`pwm(1500.0) duration exceeds the 1000us (1mS) maximum laser pulse width`)
but the value is still recorded. Expands to a `#` comment placeholder in the
layer's action block:
...
```python
driver.select_laser(1)
# Produces: LASER_DEVICE_1
```

Only laser head 1 is currently wired in rpascript. Selecting any other head
logs a warning
...
`home()` expands to two rpascript lines (jogging the X and Y axes to the
origin reference), while `home_z`/`home_u` expand to a single line each:

```python
driver.home()     # Produces: SPEED_LASER_1 <jog_xy_speed>, JOG_XY Rel:MACHINE X=0mm Y=0mm
...
|--------|------------|-------------|
| `home()` | `SPEED_LASER_1 <jog_xy_speed>`, `JOG_XY Rel:MACHINE X=0mm Y=0mm` | Jog X and Y axes to the current origin reference |
...
> **Note:** Over RPC the `get_gluescript()`/`get_rpascript()` methods and the
> `driver.rpascript` attribute report the server's last-flushed state (see the
> integration guide §3.8 batching section). The `driver.gluescript` attribute,
> by contrast, is the driver's live local transcript of what has been authored
...
> /list script
   0: # Job: My Job
   1: # Generated by: GlueScript 0.20.0
   2: REF_POINT_MACHINE
   3: SET_ABSOLUTE
   4: REF_POINT_SET
   5: START_JOB
...
|------|---------|----------|-------|
| Near | `MOVE_NEAR_XY` | Signed 14-bit / 1000 | −8.192…+8.191mm relative |
...
### Single-Axis Far Cuts and Moves

A far single-axis operation — a `cut_x_to`/`cut_y_to` or `move_x_to`/
`move_y_to` whose single-axis delta falls outside the near range — currently
emits the two-axis FAR command (`CUT_FAR_XY`/`MOVE_FAR_XY`) with the other axis
held at its current position. This is a work-around: the protocol has no
single-axis FAR cut opcodes (`CUT_FAR_X`/`CUT_FAR_Y` have not yet been
discovered), and the single-axis FAR move opcodes (`MOVE_FAR_X`/`MOVE_FAR_Y`)
do not function as expected on the controller, so the operation is encoded as
the two-axis FAR form. For example, `cut_x_to(200.0)` from the origin emits
`CUT_FAR_XY X=200.000mm Y=0.000mm`, and `cut_y_to(150.0)` after
`move_xy_to(10.0, 0.0)` emits `CUT_FAR_XY X=10.000mm Y=150.000mm`. Likewise,
`move_x_to(200.0)` from the origin emits `MOVE_FAR_XY X=200.000mm Y=0.000mm`,
and `move_y_to(150.0)` after `move_xy_to(10.0, 0.0)` emits
`MOVE_FAR_XY X=10.000mm Y=150.000mm`.

Because the work-around reuses the two-axis command, such operations appear in
decode output as two-axis `CUT_FAR_XY`/`MOVE_FAR_XY` segments — straight
horizontal or vertical — byte-indistinguishable from an explicit
`cut_xy_to(x, current_y)`/`move_xy_to(x, current_y)`. Near single-axis
operations are unaffected: they still emit `CUT_NEAR_X nearX=...` /
`CUT_NEAR_Y nearY=...` and `MOVE_NEAR_X nearX=...` / `MOVE_NEAR_Y nearY=...`.
...
```
# Job: Demo
# Generated by: GlueScript 0.20.0
REF_POINT_MACHINE
SET_ABSOLUTE
REF_POINT_SET
START_JOB
...
actions where each layer's block is prefixed with `SELECT_LAYER Layer:{n}` and is
immediately followed by that layer's overscan line(s), then the actions.
