...
> /list script
   0: # Job: My Job
   1: # Generated by: GlueScript 0.18.0
   2: REF_POINT_MACHINE
   3: SET_ABSOLUTE
   4: REF_POINT_SET
   5: START_JOB
...
### Single-Axis Far Cuts

A far single-axis cut — a `cut_x_to`/`cut_y_to` whose single-axis delta falls
outside the near range — currently emits the two-axis `CUT_FAR_XY` command with
the other axis held at its current position. This is a work-around: the
protocol has no single-axis FAR cut opcodes (`CUT_FAR_X`/`CUT_FAR_Y` have not
yet been discovered), so the cut is encoded as `CUT_FAR_XY`. For example,
`cut_x_to(200.0)` from the origin emits `CUT_FAR_XY X=200.000mm Y=0.000mm`,
and `cut_y_to(150.0)` after `move_xy_to(10.0, 0.0)` emits
`CUT_FAR_XY X=10.000mm Y=150.000mm`. The move side needs no such work-around:
`MOVE_FAR_X`/`MOVE_FAR_Y` exist in the protocol table, so `move_x_to`/
`move_y_to` emit them natively.

Because the work-around reuses the two-axis command, such cuts appear in decode
output as two-axis `CUT_FAR_XY` segments — straight horizontal or vertical —
byte-indistinguishable from an explicit `cut_xy_to(x, current_y)`. Near
single-axis cuts are unaffected: they still emit `CUT_NEAR_X nearX=...` /
`CUT_NEAR_Y nearY=...`.
...
```
# Job: Demo
# Generated by: GlueScript 0.18.0
REF_POINT_MACHINE
SET_ABSOLUTE
REF_POINT_SET
START_JOB
